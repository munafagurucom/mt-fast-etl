# Source Extraction Guide: Dwolla

## 1. Extraction Strategy
Extracting data continuously from **Dwolla** inside a Rust application requires resilient architecture. 
...
## 5. Output Formats
Data arrives from Dwolla usually in:
...
1. **Never Panic:** If the network drops or Dwolla denies authorization, implement an exponential backoff retry loop (`reqwest-retry` or native loop). A panic brings down the whole continuous streamer pipeline.
2. **Backpressure:** If your Rust application processes the Dwolla data slower than it arrives, you will face OOM (Out Of Memory) exceptions if you dump it into uncontrolled `mpsc` Tokio channels. Always use bounded channels (e.g., `mpsc::channel(1000)`).
