# Source Extraction Guide: Amazon S3

## 1. Extraction Strategy
Extracting data continuously from **Amazon S3** inside a Rust application requires resilient architecture. 
...
- **Rust Crates/SDKs:** `aws-sdk-s3`, `object_store`
- **Primary API:** S3 GetObject / ListObjectsV2
...
## 5. Output Formats
Data arrives from Amazon S3 usually in:
...
1. **Never Panic:** If the network drops or Amazon S3 denies authorization, implement an exponential backoff retry loop (`reqwest-retry` or native loop). A panic brings down the whole continuous streamer pipeline.
2. **Backpressure:** If your Rust application processes the Amazon S3 data slower than it arrives, you will face OOM (Out Of Memory) exceptions if you dump it into uncontrolled `mpsc` Tokio channels. Always use bounded channels (e.g., `mpsc::channel(1000)`).
