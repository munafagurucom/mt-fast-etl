# Source Extraction Guide: Amazon Kinesis

## 1. Extraction Strategy
Extracting data continuously from **Amazon Kinesis** inside a Rust application requires resilient architecture. 
...
- **Rust Crates/SDKs:** `aws-sdk-kinesis`
- **Primary API:** AWS XML/REST API
...
## 5. Output Formats
Data arrives from Amazon Kinesis usually in:
...
1. **Never Panic:** If the network drops or Amazon Kinesis denies authorization, implement an exponential backoff retry loop (`reqwest-retry` or native loop). A panic brings down the whole continuous streamer pipeline.
2. **Backpressure:** If your Rust application processes the Amazon Kinesis data slower than it arrives, you will face OOM (Out Of Memory) exceptions if you dump it into uncontrolled `mpsc` Tokio channels. Always use bounded channels (e.g., `mpsc::channel(1000)`).
