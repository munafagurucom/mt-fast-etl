# Source Extraction Guide: Apache Iceberg

## 1. Extraction Strategy
Extracting data continuously from **Apache Iceberg** inside a Rust application requires resilient architecture. 
...
- **Rust Crates/SDKs:** `iceberg-rust`
- **Primary API:** S3/FS Metadata reads
...
## 5. Output Formats
Data arrives from Apache Iceberg usually in:
...
1. **Never Panic:** If the network drops or Apache Iceberg denies authorization, implement an exponential backoff retry loop (`reqwest-retry` or native loop). A panic brings down the whole continuous streamer pipeline.
2. **Backpressure:** If your Rust application processes the Apache Iceberg data slower than it arrives, you will face OOM (Out Of Memory) exceptions if you dump it into uncontrolled `mpsc` Tokio channels. Always use bounded channels (e.g., `mpsc::channel(1000)`).
