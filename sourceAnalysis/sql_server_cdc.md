# Source Extraction Guide: Sql Server Cdc

## 1. Extraction Strategy
Extracting data continuously from **Sql Server Cdc** inside a Rust application requires resilient architecture. 
...
- **Rust Crates/SDKs:** `tiberius`
- **Primary API:** Change Tracking tables & Service Broker
...
## 5. Output Formats
Data arrives from Sql Server Cdc usually in:
...
1. **Never Panic:** If the network drops or Sql Server Cdc denies authorization, implement an exponential backoff retry loop (`reqwest-retry` or native loop). A panic brings down the whole continuous streamer pipeline.
2. **Backpressure:** If your Rust application processes the Sql Server Cdc data slower than it arrives, you will face OOM (Out Of Memory) exceptions if you dump it into uncontrolled `mpsc` Tokio channels. Always use bounded channels (e.g., `mpsc::channel(1000)`).
