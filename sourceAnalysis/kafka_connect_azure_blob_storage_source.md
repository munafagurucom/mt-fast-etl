# Source Extraction Guide: Kafka Connect Azure Blob Storage Source

## 1. Extraction Strategy
Extracting data continuously from **Kafka Connect Azure Blob Storage Source** inside a Rust application requires resilient architecture. 
...
- **Rust Crates/SDKs:** `azure_storage_blobs`
- **Primary API:** Azure Storage REST API
...
## 5. Output Formats
Data arrives from Kafka Connect Azure Blob Storage Source usually in:
...
1. **Never Panic:** If the network drops or Kafka Connect Azure Blob Storage Source denies authorization, implement an exponential backoff retry loop (`reqwest-retry` or native loop). A panic brings down the whole continuous streamer pipeline.
2. **Backpressure:** If your Rust application processes the Kafka Connect Azure Blob Storage Source data slower than it arrives, you will face OOM (Out Of Memory) exceptions if you dump it into uncontrolled `mpsc` Tokio channels. Always use bounded channels (e.g., `mpsc::channel(1000)`).
