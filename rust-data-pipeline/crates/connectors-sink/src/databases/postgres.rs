...
impl TryFrom<&ConnectorConfig> for PostgresSinkConfig {
    ...
    fn try_from(config: &ConnectorConfig) -> Result<Self, Self::Error> {
        ...
        let connection_string = config.secrets.get("connection_string").cloned()
            .or_else(|| properties.get("connection_string").and_then(|v| v.as_str()).map(String::from))
        ...
        Ok(PostgresSinkConfig {
            connection_string,
            table_name: properties.get("table_name").and_then(|v| v.as_str()).map(String::from)
                .ok_or_else(|| PipelineError::ConfigError {
                    message: "Missing required property: table_name".to_string(),
                })?,
            schema_name: properties.get("schema_name").and_then(|v| v.as_str()).map(String::from),
...
pub struct PostgresSink {
    ...
    // No explicit caching of PgStatement, handled by sqlx connection pool
}
...
impl PostgresSink {
    ...
    // convert_arrow_value_to_sql removed
}
...
impl SinkConnector for PostgresSink {
    ...
    async fn write_batch(&mut self, batch: &RecordBatch) -> Result<WriteReceipt, PipelineError> {
        ...
        let insert_sql = self.build_insert_statement(schema.as_ref());
                ...
                for (col_idx, field) in schema.fields().iter().enumerate() {
                    ...
                    match field.data_type() {
                        ...
                        DataType::Timestamp(_, _) => {
                            ...
                            query = query.bind(datetime.to_rfc3339());
                        }
                        _ => {
                            // Fallback to string
